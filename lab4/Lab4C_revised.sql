...
BEGIN
IF ((SELECT reservation_number FROM reservation WHERE reservation_number = reservation.reservation_number) IS NULL) THEN
	SELECT "Reservation number doesn't exist" AS "Message";
ELSEIF((SELECT passport_number FROM passenger_reservation WHERE reservation_number = passenger_reservation.reservation_number AND passport_number = passenger_reservation.passport_number) IS NULL) THEN
	SELECT "Passenger doesn't exist on reservation" AS "Message";
ELSEIF ((SELECT passport_number FROM contact WHERE contact.email = email AND contact.passport_number = passport_number) = passport_number) THEN
	UPDATE reservation
    SET contact := passport_number
    WHERE reservation.reservation_number = reservation_number;
    SELECT "OK. Added reservation to existing contact" as "Message";
